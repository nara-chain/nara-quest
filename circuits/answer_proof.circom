...
//
// round binds the proof to a specific quest round,
// preventing proof replay attacks across rounds with the same answer_hash.
...
template AnswerProof() {
    ...
    signal input pubkey_lo;     // Lower 128 bits of user's pubkey
    signal input pubkey_hi;     // Upper 128 bits of user's pubkey
    signal input round;         // On-chain round number (prevents cross-round replay)
    ...
    // Bind proof to user's pubkey and round (these are public inputs, so they
    ...
    signal round_bind;
    round_bind <== round * pubkey_lo;
}

component main {public [answer_hash, pubkey_lo, pubkey_hi, round]} = AnswerProof();
